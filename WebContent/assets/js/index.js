...
$(function() {
	...
	//获取城市天气
	 ...
	 $(document).delegate("#shrinkage_new","click",function(){
		 ...
		 //移除顶部title
		 $("#logo_top_left_combination").html('');
	 });
...
function leftCollapse(){
	//按钮位置切换
	 ...
	 $("#shrinkage").html('');
	 //title切换为顶部菜单title
	 $("#logo_top_left_combination").html('<img src="'+ basePath +'resource/images/index/logo.png" alt=""/><img src="'+ basePath +'resource/images/index/title.png" alt=""/>');
}
// 登出
function logout() {
	$.ajax({
...
function getWeather(){
	$.getJSON(basePath+'appConfig.json', function (data){
		appConfigParam=data;
		var isShow=appConfigParam.cityWeather.showWeather;
		var cityCode=appConfigParam.cityWeather.cityCode;
		if(isShow==undefined ||isShow==null ||isShow.toLowerCase()=="false" || cityCode==undefined ||cityCode==null)
			return;
...
function getWeatherData(cityCode) {
	$.ajax({
		type : 'get',
		url : 'http://restapi.amap.com/v3/weather/weatherInfo?city='+cityCode+'&key=a0f65e41a50b84fb2e165f1fd3144a37&extensions=base',
		dataType : 'json',
		success : function(data) {
			if(data != null && data.lives != null && data.lives.length > 0 && data.lives[0].weather != null && data.lives[0].weather != null){
				var weatherTable = '<table style="width:150px;line-height:68px;text-align:center;color: white;font-weight: bolder;font-size: 15px;">';
					weatherTable += '<tr>'
...
function getMenuData() {
	$.ajax({
		type : 'post',
		url : basePath + 'uv/uvaction/getfuncbyuser.action',
		dataType : 'json',
		success : function(data) {
			// 进行数据转换
			var dataMenu = data;
